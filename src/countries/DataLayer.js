...
define([
	...
	function(ww) {
		 ...
		 *@param {Array of GlobalDataPoints} dataArray, a DataArray containing global data points
		 *@param {String} flagAddress, a string indicating the address of the flags avaliable
		 */
		DataLayer.prototype.loadFlags = function(dataArray, flagAddress, 
			...
			for(i = 0; i < dataArray.length; i++) {
				...
				var placemark = new WorldWind.Placemark(new
                            WorldWind.Position(dataArray[i].latitude,
								dataArray[i].longitude, 1e2), true, null);
                placemark.label = dataArray[i].name;
                placemark.altitudeMode = WorldWind.RELATIVE_TO_GROUND;
                placemarkAttributes.imageSource = flagAddress + 
					dataArray[i].options.icon_code + '.png';

				placemark.highlightAttributes = highlightAttributes;

				//Attach the type to it
				placemark.type = this._name;
				...
				placemark.userObject = dataArray[i].options;
